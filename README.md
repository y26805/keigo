...
|---|---|---|
| 催促 (chasing a reply) | `進捗どう` | 本件の進捗につきまして、その後いかがでしょうか。ご多忙のところ恐れ入りますが、現在の状況をお知らせいただけますでしょうか。 |
| セールストークの拒否 (declining a pitch) | `今はいらない` | せっかくのご提案ですが、今回は導入を見送らせていただきます。今後必要になりました際には、改めてご連絡いたします。 |
| 発注見送り (going with a competitor) | `本件は予算で発注できない。残念だけどアルファ商事にする予定` | この度はご提案いただき、誠にありがとうございました。社内で検討いたしました結果、予算面の都合により、今回はアルファ商事にお願いする運びとなりました。誠に恐縮ですが、ご了承いただけますでしょうか。 |
